...
class Firmware():
    def __init__(self, 
        path_to_module_folder:str,
        path_to_project_config:str
        ) -> None:
    ...
    def parse(self) -> int:
        ...
        if self.path_to_project_config is not None:
            ...
            self.project_config = {"project name":"", "version":"", "copyright notice":""}
                ...
                with open(os.path.join(path_to_folder, filename), 'r') as config_file:
                    ...
                    for request in config_parser.requests.values():
                        if request.name in self.requests:
                            logging.error(f"Request with name '{request.name}' already exists!")
                            ret += 1
    ...
    def create_kernels(self) -> int:
        ...
        for config_parser in self.config_parsers.values():
            ...
            if kernel_number not in self.kernels:
                kernel = Kernel(kernel_number=kernel_number)
                kernel.add_module(config_parser.module)
                self.kernels[kernel_number] = kernel
            else:
    ...
    def system_info(self) -> None:
        ...
        handler_limit = max(max_handlers.values())
