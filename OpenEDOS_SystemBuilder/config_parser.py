...
class ConfigParser():
    def __init__(self,
        ...
        self.path_to_config:str = os.path.join(
            path_to_folder, 
            config_filename)
    ...
    def check_config(self) -> None:
        ...
        if "author" not in self.config:
            logging.debug(f"Config '{self.name}': 'author' not specified. Setting to 'Anonymous'...")
        ...
        if "create" not in self.interface_config:
            logging.debug(f"Config '{self.name}': 'interface': 'create' not specified. Setting to False...")
            self.interface_config["create"] = False
        if "requests" not in self.interface_config:
            logging.debug(f"Config '{self.name}': 'interface': 'create' not specified. Setting to None...")
        if "module" not in self.config:
            logging.debug(f"Config '{self.name}': 'module' not specified!")
            self.config["module"] = {"create":False, "kernel":0, "subscribed requests":None, "used requests":None}
        self.module_config = self.config["module"]
        if "create" not in self.module_config:
            logging.debug(f"Config '{self.name}': 'module': 'create' not specified. Setting to False...")
            self.module_config["create"] = False
        if "kernel" not in self.module_config:
            logging.debug(f"Config '{self.name}': 'module': 'kernel' not specified. Setting to 0...")
            self.module_config["kernel"] = 0
        if "subscribed requests" not in self.module_config:
            logging.debug(f"Config '{self.name}': 'module': 'subscribed requests' not specified. Setting to None...")
    ...
    def parse_config(self) -> int:
        ...
        if self.path_to_folder.endswith(self.name):
        ...
        self.path_to_module_header = os.path.join(self.path_to_folder, f"{filename}_mod.h")
        self.path_to_module_source = os.path.join(self.path_to_folder, f"{filename}_mod.c")
        self.path_to_interface_header = os.path.join(self.path_to_folder, f"{filename}_intf.h")
        self.path_to_interface_source = os.path.join(self.path_to_folder, f"{filename}_intf.c")
        ...
        if request_configs is not None:
            for request_config in request_configs:
                ...
                if name in self.requests:
                    logging.error(f"Config '{self.name}': Multiple definitions of request '{name}'!")
                    ret += 1
                    continue
                ...
                if request_parameter_configs is not None:
                    for request_parameter_config in request_parameter_configs:
                        ...
                        if request_parameter_name in request_parameters:
                            logging.error(f"Config '{self.name}': Request '{name}':\n"\
                ...
                if has_response:
                    logging.debug(f"Config '{self.name}': Request '{name}': Creating response")
                    ...
                    if response_parameter_configs is not None:
                        for response_parameter_config in response_parameter_configs:
                            ...
                            debug += f"Parameter type: {response_parameter_config['type']}\n"
                            debug += f"Parameter description: {response_parameter_config['description']}"
                            logging.debug(debug)
                            response_parameters[response_parameter_name] = response_parameter
    ...
    def create_module(self, 
        ...
        if not self.module_config["create"]:
            logging.debug(f"Config '{self.name}': Skip creating module")
            return ret
    ...
    def create_request_handlers(self, 
        ...
        if handler_configs is not None:
            ...
            for handler_config in handler_configs:
                ...
                logging.debug(f"Config '{self.name}': Creating request handler for subscribed request '{name}'")
                if name in self.subscribed_requests:
                    logging.error(f"Config '{self.name}': Subscribed request '{name}' is already listed!")
                    ret += 1
                    continue
                if not (name in all_requests):
                    logging.warning(f"Config '{self.name}': Subscribed request '{name}' is not defined!")
                    request = Request(name)
                    #all_requests[name] = dummy
                    ret += 1
                else:
    ...
    def create_response_handlers(self, 
        all_requests:Dict[str, Request]
        ) -> int:
        ret = 0
        ...
        if handler_configs is not None:
            ...
            for handler_config in handler_configs:
                ...
                if name in self.used_requests:
                    logging.error(f"Config '{self.name}': Used request '{name}' is already listed!")
                    ret += 1
                    continue
                if not (name in all_requests):   
                    logging.warning(f"Config '{self.name}': Used request '{name}' is not defined!") 
                    request = Request(name)
                    ret += 1
                else:               
                    request = all_requests[name]
                ...
                if request.has_response:
                    logging.debug(f"Config '{self.name}': Creating response handler for used request '{name}'")
