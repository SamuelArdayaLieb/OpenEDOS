...
def add_request(
    ...
    if defined:
        ...
        if request.has_response:
            request_dict["response_description"] = request.response_description
...
def read_config(
    ...
    if len(filelist) == 0:
        logging.error(f"Could not find _config.yaml file in folder\n{path_to_folder}!")
        return None, None
    if len(filelist) > 1:
        logging.error(f"Found more than one _config.yaml files in folder\n{path_to_folder}!")
        return None, None
    filename = filelist[0]    
